    ...
    <div class="bg-[#eef0ec] py-8"><div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('cases.store') }}" class="bg-white p-6 shadow-sm">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div><label class="mb-2 block text-sm font-bold uppercase text-[#554b45]">Assigned Lawyer</label><select name="assigned_lawyer_id" class="w-full border-[#c1c1bd] bg-white text-[#030203] focus:border-[#9f7957] focus:ring-[#9f7957]"><option value="">Select lawyer</option>@foreach($lawyers as $lawyer)<option value="{{ $lawyer->id }}" @selected(old('assigned_lawyer_id') == $lawyer->id)>{{ $lawyer->full_name }}</option>@endforeach</select></div>
                <div><label class="mb-2 block text-sm font-bold uppercase text-[#554b45]">Case Type</label><input type="text" name="case_type" value="{{ old('case_type') }}" class="w-full border-[#c1c1bd] bg-white text-[#030203] focus:border-[#9f7957] focus:ring-[#9f7957]"></div>
