<x-app-layout>
    <x-slot name="header"><div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-sm font-bold uppercase text-[#9f7957]">File room</p><h2 class="mt-2 text-3xl font-extrabold text-[#030203]">Document</h2></div>@unless(Auth::user()?->isLawyer())<a href="{{ route('documents.edit', $document->id) }}" class="inline-flex items-center justify-center bg-[#030203] px-4 py-2 text-sm font-bold text-white transition hover:bg-[#554b45]">Edit document</a>@endunless</div></x-slot>
    <div class="bg-[#eef0ec] py-8"><div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
        <section class="bg-white p-6 shadow-sm">
            ...
            <div class="mt-6 flex items-center justify-between border-t border-[#e3e3df] pt-5">
                ...
                @if($document->file_path)<a href="{{ route('documents.download', $document) }}" class="bg-[#c7a47b] px-4 py-2 text-sm font-bold text-[#030203] hover:bg-[#030203] hover:text-white">Download file</a>@endif
            </div>
        </section>
    </div></div>
</x-app-layout>
