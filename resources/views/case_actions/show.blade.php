<x-app-layout>
    <x-slot name="header"><div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-sm font-bold uppercase text-[#9f7957]">Matter tasks</p><h2 class="mt-2 text-3xl font-extrabold text-[#030203]">Case Action</h2></div>@unless(Auth::user()?->isLawyer())<a href="{{ route('case-actions.edit', $caseAction->id) }}" class="inline-flex items-center justify-center bg-[#030203] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#554b45]">Edit action</a>@endunless</div></x-slot>
