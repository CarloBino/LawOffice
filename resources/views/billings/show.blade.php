<x-app-layout>
    <x-slot name="header"><div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-sm font-bold uppercase text-[#9f7957]">Accounts and fees</p><h2 class="mt-2 text-3xl font-extrabold text-[#030203]">{{ Auth::user()?->isLawyer() ? 'Billing Details' : 'Manage Billing' }}</h2></div>@unless(Auth::user()?->isLawyer())<a href="#record-payment" class="inline-flex items-center justify-center bg-[#030203] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#554b45]">Record payment</a>@endunless</div></x-slot>
    <div class="bg-[#eef0ec] py-8"><div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8"><div class="bg-white shadow-sm">
        ...
        <div class="grid gap-0 sm:grid-cols-2">
            ...
            <div class="border-b border-[#e3e3df] p-5 sm:border-r"><p class="text-xs font-bold uppercase text-[#9f7957]">Latest Date Received</p><p class="mt-2 font-semibold text-[#030203]">{{ $billing->payment_date ? \Illuminate\Support\Carbon::parse($billing->payment_date)->format('M d, Y') : 'No payment received' }}</p></div>
            ...
            <div class="border-b border-[#e3e3df] p-5 sm:col-span-2"><p class="text-xs font-bold uppercase text-[#9f7957]">Related Hearing</p><p class="mt-2 font-semibold text-[#030203]">@if($billing->hearing)<a href="{{ route('hearings.show', $billing->hearing_id) }}" class="hover:text-[#9f7957]">{{ $billing->hearing->hearing_date ? \Illuminate\Support\Carbon::parse($billing->hearing->hearing_date)->format('M d, Y') : 'TBA' }} / {{ $billing->hearing->hearing_purpose ?: 'Hearing' }}</a>@else Case-level billing @endif</p></div>
        </div>
